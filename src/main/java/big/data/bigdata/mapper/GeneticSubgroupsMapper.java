...
public interface GeneticSubgroupsMapper {
    // 根据染色体编号和位置查询 GeneticSubgroups，并支持分页
    @SelectProvider(type = GeneticSubgroupsSqlProvider.class, method = "selectGeneticSubgroupsByChromosomeAndPosition")
    @Results({
    ...
    List<GeneticSubgroups> findGeneticSubgroupsByChromosomeAndPosition(
            @Param("tableName") String tableName, //可以使用#{tableName}使用这个参数
    ...
    // 查询总记录数
    @SelectProvider(type = GeneticSubgroupsSqlProvider.class, method = "countGeneticSubgroupsByChromosomeAndPosition")
    long countGeneticSubgroupsByChromosomeAndPosition(
